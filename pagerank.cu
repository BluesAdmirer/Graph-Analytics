...
int main()
{
	...
	if(optident==1 && optchain==0 && optdead==0)
	{
		...
		for(i=0;i<par.size()-1;i++)
		{
			...
			int szz=0;
			for(int i=0;i<com;i++){
				memsz[i]=members[order[i]].size();
				szz+=members[order[i]].size();
			}
			...
			int temp[com];
			for(int i=0;i<com;i++){
				if(i) temp[i]=temp[order[i-1]]+memsz[order[i-1]];
				else temp[i]=0;
			}
			int kk=0;
			int mem[szz];
			for(int i=0;i<com;i++){
				for(int c:members[order[i]]){
					mem[kk++]=c;
				}
			}
			...
			int tempg[n];
			for(int i=0;i<n;i++){
				if(i) tempg[i]=tempg[i-1]+rcwgraph[i-1].size();
				else tempg[i]=0;
			}
...
// 				long long val=computeranki(rcgraph,alt,parent,left[order[j]],members[order[j]].size(),outdeg,members[order[j]],rank,initial);
		...
		// 	fout << initial[i] << "\n";
		// }
		cudaMemcpy(cinitial, initial, n*sizeof(int),cudaMemcpyHostToDevice);
		update<<<10,100>>>(cinitial,cn);
		// cudaDeviceSynchronize();
		cudaMemcpy(initial, cinitial, n*sizeof(int),cudaMemcpyDeviceToHost);
		double sum=0;
		for(i=0;i<n;i++)
			sum=sum+rank[i];
		for(i=0;i<n;i++) rank[i]=rank[i]/sum, initial[i]=0.85*initial[i];
		for(i=0;i<n;i++){
			fout << initial[i] << "\n";
		}
